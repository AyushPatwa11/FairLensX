...
python -m venv venv
echo Upgrading pip and installing dependencies into the venv...
IF EXIST venv\Scripts\python.exe (
    call venv\Scripts\python.exe -m pip install --upgrade pip
    call venv\Scripts\python.exe -m pip install -r requirements.txt
    set VENV_PYTHON=venv\Scripts\python.exe
) ELSE (
    echo [ERROR] Python venv creation failed or this system layout is unexpected.
    pause
    exit /b
)
IF %ERRORLEVEL% NEQ 0 (
    echo [ERROR] Failed to install requirements. See messages above.
    pause
    exit /b
)
cd ..
